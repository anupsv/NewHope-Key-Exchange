...
class NewHope:
    ...
    def keygen(self, send, sendOffset):

        a = _Poly()
        e = _Poly()
        r = _Poly()
        pk = _Poly()
        seed = [0] * (NewHope.SEEDBYTES + 32)
        noiseseed = [0] * 32

        try:
            NewHope.randombytes(seed)
            NewHope.sha3256(seed, 0, seed, 0, NewHope.SEEDBYTES)  # / * Don't send output of system RNG */
            System.arraycopy(seed, NewHope.SEEDBYTES, noiseseed, 0, 32)

            NewHope.uniform(a.coeffs, seed)
            if sk == Null:
                sk = _Poly()
            sk.getnoise(noiseseed, 0)
            sk.ntt()
            ...
            NewHope.encode_a(send, sendOffset, pk, seed)
        finally:
            a.destroy()
            e.destroy()
            r.destroy()
    ...
    @staticmethod
    def shake128_absorb(s, _input, inputOffset, inputByteLen):
        NewHope.keccak_absorb(s, NewHope.SHAKE128_RATE, _input, inputOffset, inputByteLen, 0x1F)

    @staticmethod
    def shake128_squeezeblocks(output, outputOffset, nblocks, s):
        NewHope.keccak_squeezeblocks(output, outputOffset, nblocks, s, NewHope.SHAKE128_RATE)

    @staticmethod
    def uniform(coeffs, seed):
        pos = 0
        ctr = 0
        state = [0] * 25
        nblocks = 14
        buf = [0] * (NewHope.SHAKE128_RATE * nblocks)

        try:
            NewHope.shake128_absorb(state, seed, 0, NewHope.SEEDBYTES)
            NewHope.shake128_squeezeblocks(buf, 0, nblocks, state)

            while ctr < NewHope.PARAM_N:
                val = ((buf[pos] & 0xff) | ((buf[pos + 1] & 0xff) << 8))
                if val < 5 * NewHope.PARAM_Q:
                    coeffs[ctr] = val
                    ctr += 1
                pos += 2
                if pos > NewHope.SHAKE128_RATE * nblocks - 2:
                    nblocks = 1
                    NewHope.shake128_squeezeblocks(buf, 0, nblocks, state)
                    pos = 0
    ...
    @staticmethod
    def randombytes(buffer):
        return urandom(buffer)

    @staticmethod
    def encode_a(r, roffset, pk, seed):
        pk.tobytes(r, roffset)
        i = 0
        while i < NewHope.SEEDBYTES:
            r[NewHope.POLY_BYTES + roffset + i] = seed[i]

    @staticmethod
    def decode_a(pk, seed, r, roffset):
        pk.frombytes(r, roffset)
        i = 0
        while i < NewHope.SEEDBYTES:
            seed[i] = r[NewHope.POLY_BYTES + roffset + i]
            i += 1

    @staticmethod
    def encode_b(r, roffset, b, c):
        b.tobytes(r, roffset)
        i = 0
        while i < (NewHope.PARAM_N / 4):
            r[NewHope.POLY_BYTES + roffset + i] = (
            c.coeffs[4 * i] | (c.coeffs[4 * i + 1] << 2) | (c.coeffs[4 * i + 2] << 4) | (c.coeffs[4 * i + 3] << 6));
            i += 1

    @staticmethod
    def decode_b(b, c, r, roffset):
        b.frombytes(r, roffset)
        i = 0
        while i < (NewHope.PARAM_N / 4):
            c.coeffs[4 * i + 0] = (r[NewHope.POLY_BYTES + roffset + i] & 0x03)
            c.coeffs[4 * i + 1] = ((r[NewHope.POLY_BYTES + roffset + i] >> 2) & 0x03)
            c.coeffs[4 * i + 2] = ((r[NewHope.POLY_BYTES + roffset + i] >> 4) & 0x03)
            c.coeffs[4 * i + 3] = ((r[NewHope.POLY_BYTES + roffset + i] & 0xff) >> 6)
            i += 1

    # // -------------- ntt.c --------------
    bitrev_table_combined = [
    ...
    @staticmethod
    def bitrev_vector(poly):
        p = 0
        while p < 496:
            indices = NewHope.bitrev_table_combined[p]
            i = indices & 0x03FF
            r = indices >> 10
            tmp = poly[i]
            poly[i] = poly[r]
    ...
    @staticmethod
    def mul_coefficients(poly, factors):
        i = 0
        while i < NewHope.PARAM_N:
            poly[i] = NewHope.montgomery_reduce((poly[i] * factors[i]))
            i += 1

    @staticmethod
    # GS_bo_to_no; omegas need to be in Montgomery domain
    def ntt_global(a, omega):
        ...
        while i < 10:
            ...
            while start < distance:
                ...
                while j < (NewHope.PARAM_N - 1):
                    jTwiddle += 1
                    W = omega[jTwiddle]
                    temp = a[j]
                    a[j] = (temp + a[j + distance])  # Omit reduction(be lazy)
                    a[j + distance] = NewHope.montgomery_reduce((W * (temp + 3 * PARAM_Q - a[j + distance])))
            ...
            while start < distance:
                ...
                while j < (NewHope.PARAM_N - 1):
                    ...
                    temp = a[j]
                    j += 2 * distance
                    a[j] = NewHope.barrett_reduce((temp + a[j + distance]))
                    a[j + distance] = NewHope.montgomery_reduce((W * (temp + 3 * PARAM_Q - a[j + distance])))
                start += 1
    ...
    def KeccakF1600_StatePermute(state):
        ...
        while round < 24:
            ...
            Da = BCu ^ NewHope.ROL(BCe, 1)
            De = BCa ^ NewHope.ROL(BCi, 1)
            Di = BCe ^ NewHope.ROL(BCo, 1)
            Do = BCi ^ NewHope.ROL(BCu, 1)
            Du = BCo ^ NewHope.ROL(BCa, 1)

            Aba ^= Da
            BCa = Aba
            Age ^= De
            BCe = NewHope.ROL(Age, 44)
            Aki ^= Di
            BCi = NewHope.ROL(Aki, 43)
            Amo ^= Do
            BCo = NewHope.ROL(Amo, 21)
            Asu ^= Du
            BCu = NewHope.ROL(Asu, 14)
            Eba = BCa ^ ((~BCe) & BCi)
            Eba ^= NewHope.KeccakF_RoundConstants[round]
            Ebe = BCe ^ ((~BCi) & BCo)
            Ebi = BCi ^ ((~BCo) & BCu)
            Ebo = BCo ^ ((~BCu) & BCa)
            Ebu = BCu ^ ((~BCa) & BCe)
            ...
            Abo ^= Do
            BCa = NewHope.ROL(Abo, 28)
            Agu ^= Du
            BCe = NewHope.ROL(Agu, 20)
            Aka ^= Da
            BCi = NewHope.ROL(Aka, 3)
            Ame ^= De
            BCo = NewHope.ROL(Ame, 45)
            Asi ^= Di
            BCu = NewHope.ROL(Asi, 61)
            Ega = BCa ^ ((~BCe) & BCi)
            Ege = BCe ^ ((~BCi) & BCo)
            Egi = BCi ^ ((~BCo) & BCu)
            Ego = BCo ^ ((~BCu) & BCa)
            ...
            Abe ^= De
            BCa = NewHope.ROL(Abe, 1)
            Agi ^= Di
            BCe = NewHope.ROL(Agi, 6)
            Ako ^= Do
            BCi = NewHope.ROL(Ako, 25)
            Amu ^= Du
            BCo = NewHope.ROL(Amu, 8)
            Asa ^= Da
            BCu = NewHope.ROL(Asa, 18)
            Eka = BCa ^ ((~BCe) & BCi)
            Eke = BCe ^ ((~BCi) & BCo)
            Eki = BCi ^ ((~BCo) & BCu)
            Eko = BCo ^ ((~BCu) & BCa)
            ...
            Abu ^= Du
            BCa = NewHope.ROL(Abu, 27)
            Aga ^= Da
            BCe = NewHope.ROL(Aga, 36)
            Ake ^= De
            BCi = NewHope.ROL(Ake, 10)
            Ami ^= Di
            BCo = NewHope.ROL(Ami, 15)
            Aso ^= Do
            BCu = NewHope.ROL(Aso, 56)
            Ema = BCa ^ ((~BCe) & BCi)
            Eme = BCe ^ ((~BCi) & BCo)
            Emi = BCi ^ ((~BCo) & BCu)
            Emo = BCo ^ ((~BCu) & BCa)
            ...
            Abi ^= Di
            BCa = NewHope.ROL(Abi, 62)
            Ago ^= Do
            BCe = NewHope.ROL(Ago, 55)
            Aku ^= Du
            BCi = NewHope.ROL(Aku, 39)
            Ama ^= Da
            BCo = NewHope.ROL(Ama, 41)
            Ase ^= De
            BCu = NewHope.ROL(Ase, 2)
            Esa = BCa ^ ((~BCe) & BCi)
            Ese = BCe ^ ((~BCi) & BCo)
            Esi = BCi ^ ((~BCo) & BCu)
            Eso = BCo ^ ((~BCu) & BCa)
            ...
            BCa = Eba
            Ege ^= De
            BCe = NewHope.ROL(Ege, 44)
            Eki ^= Di
            BCi = NewHope.ROL(Eki, 43)
            Emo ^= Do
            BCo = NewHope.ROL(Emo, 21)
            Esu ^= Du
            BCu = NewHope.ROL(Esu, 14)
            Aba = BCa ^ ((~BCe) & BCi)
            Aba ^= NewHope.KeccakF_RoundConstants[round + 1]
            Abe = BCe ^ ((~BCi) & BCo)
            Abi = BCi ^ ((~BCo) & BCu)
            Abo = BCo ^ ((~BCu) & BCa)
            Abu = BCu ^ ((~BCa) & BCe)
            ...
            Ebo ^= Do
            BCa = NewHope.ROL(Ebo, 28)
            Egu ^= Du
            BCe = NewHope.ROL(Egu, 20)
            Eka ^= Da
            BCi = NewHope.ROL(Eka, 3)
            Eme ^= De
            BCo = NewHope.ROL(Eme, 45)
            Esi ^= Di
            BCu = NewHope.ROL(Esi, 61)
            Aga = BCa ^ ((~BCe) & BCi)
            Age = BCe ^ ((~BCi) & BCo)
            Agi = BCi ^ ((~BCo) & BCu)
            Ago = BCo ^ ((~BCu) & BCa)
            ...
            Ebe ^= De
            BCa = NewHope.ROL(Ebe, 1)
            Egi ^= Di
            BCe = NewHope.ROL(Egi, 6)
            Eko ^= Do
            BCi = NewHope.ROL(Eko, 25)
            Emu ^= Du
            BCo = NewHope.ROL(Emu, 8)
            Esa ^= Da
            BCu = NewHope.ROL(Esa, 18)
            Aka = BCa ^ ((~BCe) & BCi)
            Ake = BCe ^ ((~BCi) & BCo)
            Aki = BCi ^ ((~BCo) & BCu)
            Ako = BCo ^ ((~BCu) & BCa)
            ...
            Ebu ^= Du
            BCa = NewHope.ROL(Ebu, 27)
            Ega ^= Da
            BCe = NewHope.ROL(Ega, 36)
            Eke ^= De
            BCi = NewHope.ROL(Eke, 10)
            Emi ^= Di
            BCo = NewHope.ROL(Emi, 15)
            Eso ^= Do
            BCu = NewHope.ROL(Eso, 56)
            Ama = BCa ^ ((~BCe) & BCi)
            Ame = BCe ^ ((~BCi) & BCo)
            Ami = BCi ^ ((~BCo) & BCu)
            Amo = BCo ^ ((~BCu) & BCa)
            ...
            Ebi ^= Di
            BCa = NewHope.ROL(Ebi, 62)
            Ego ^= Do
            BCe = NewHope.ROL(Ego, 55)
            Eku ^= Du
            BCi = NewHope.ROL(Eku, 39)
            Ema ^= Da
            BCo = NewHope.ROL(Ema, 41)
            Ese ^= De
            BCu = NewHope.ROL(Ese, 2)
            Asa = BCa ^ ((~BCe) & BCi)
            Ase = BCe ^ ((~BCi) & BCo)
            Asi = BCi ^ ((~BCo) & BCu)
            Aso = BCo ^ ((~BCu) & BCa)
    ...
    @staticmethod
    def keccak_absorb(s, r, m, offset, mlen, p):
        ...
        try:
            ...
            while (mlen >= r):
                i = 0
                while i < (r / 8):
                    s[i] ^= NewHope.load64(m, offset + 8 * i)
                    i += 1

                NewHope.KeccakF1600_StatePermute(s)
                mlen -= r
                offset += r
            ...
            i = 0
            while i < r / 8:
                s[i] ^= NewHope.load64(t, 8 * i)
                i += 1
        finally:
            del t

    @staticmethod
    def keccak_squeezeblocks(h, offset, nblocks, s, r):

        while nblocks > 0:
            NewHope.KeccakF1600_StatePermute(s)
            i = 0
            while i < (r >> 3):
                NewHope.store64(h, offset + 8 * i, s[i])
    ...
    @staticmethod
    def sha3256(output, outputOffset, _input, inputOffset, inputByteLen):

        s = [0] * 25
        t = [0] * NewHope.SHA3_256_RATE

        try:
            NewHope.keccak_absorb(s, NewHope.SHA3_256_RATE, _input, inputOffset, inputByteLen, 0x06)
            NewHope.keccak_squeezeblocks(t, 0, 1, s, NewHope.SHA3_256_RATE)
            i = 0
            while i < 32:
                output[i] = t[i]
                i += 1
...
class _Poly:
    ...
    def getnoise(self, seed, nonce):
        buf = [0] * (4 * NewHope.PARAM_N)

        try:
            NewHope.crypto_stream_chacha20(buf, 0, 4 * NewHope.PARAM_N, nonce, seed)
            i = 0
            while i < NewHope.PARAM_N:
    ...
    def pointwise(self, a, b):
        i = 0
        while i < NewHope.PARAM_N:
            t = NewHope.montgomery_reduce(3186 * b.coeffs[i])
            self.coeffs[i] = NewHope.montgomery_reduce(a.coeffs[i] * t)
            i += 1

    def add(self, a, b):
        i = 0
        while i < NewHope.PARAM_N:
            self.coeffs[i] = NewHope.barrett_reduce(a.coeffs[i] + b.coeffs[i])

    def ntt(self):
        NewHope.mul_coefficients(self.coeffs, NewHope.psis_bitrev_montgomery)
        NewHope.ntt_global(self.coeffs, NewHope.omegas_montgomery)

    def invntt(self):
        NewHope.bitrev_vector(self.coeffs)
        NewHope.ntt_global(self.coeffs, NewHope.omegas_inv_montgomery)
        NewHope.mul_coefficients(self.coeffs, NewHope.psis_inv_montgomery)
